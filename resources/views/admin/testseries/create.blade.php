...
@section('header')
    <header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-xl font-bold text-slate-900">Add New Test Series</h1>
        </div>
        <div class="flex items-center gap-3">
            {{-- <button
                class="px-6 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all">Publish
                Course</button> --}}
        </div>
    </header>
@endsection
...
@section('content')
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        <div class="max-w-3xl mx-auto">
            <form id="addcourse" action="{{ route('admin.testseries.store') }}" method="post"
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <div class="p-8 space-y-8">
                        ...
                        <section class="space-y-6">
                            ...
                            <div class="space-y-4">
                                <div class="space-y-2">
                                    <label class="text-sm font-semibold text-slate-700">Test  Title</label>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea name="description" rows="4"
                                        class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all resize-none">{{ old('description') }}</textarea>
                                </div>
                                <div class="space-y-2">
                        ...
                        <section class="space-y-6">
                            ...
                            {{-- <div class="space-y-2">
                                <label class="text-sm font-semibold text-slate-700">Map Test Names (Multiple)</label>
                                <div class="space-y-3">
                                    <div class="flex gap-2">
                    ...
                    <div class="px-8 py-6 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-4">
                        ...
                        <button
                            class="px-8 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 transition-all">Create
                            Course</button>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        // document.querySelector("#addcourse").addEventListener("submit", function(e) {
        //     e.preventDefault();
        //     let form = this;
        //     let formData = new FormData(form);
        //     fetch(form.action, {
        ...
        const noSubjectsHint = document.getElementById('noSubjectsHint');
        ...
        function renderSubjects() {
            if (subjects.size === 0) {
                subjectsList.innerHTML =
                    '<p id="noSubjectsHint" class="text-xs text-slate-400 italic p-2">No subjects added yet...</p>';
                return;
            }

            subjectsList.innerHTML = Array.from(subjects).map(sub => `
                <div class="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-sm font-medium border border-indigo-100 group animate-in fade-in zoom-in duration-200">
                    ${sub}
                    <button type="button" onclick="removeSubject('${sub}')" class="text-indigo-400 hover:text-indigo-600 transition-colors">
                    ...
                    <input type="text" name="subjects[]" value="${sub}" hidden/>
                </div>
            `).join('');
        }
    </script>
